...
class iBOTPatchLoss(nn.Module):
    ...
    def forward(self, student_patch_tokens, teacher_patch_tokens, student_masks_flat):
        """Compute cross-entropy between teacher and student patch tokens.

        Parameters
        ----------
        student_patch_tokens : torch.Tensor
            Student patch tokens with shape ``(B, N, D)``.
        teacher_patch_tokens : torch.Tensor
            Teacher patch tokens with shape ``(B, N, D)``.
        student_masks_flat : torch.Tensor
            Mask for valid tokens with shape ``(B, N)``.
    ...
    def apply_center_update(self):
        if self.updated is False:
            ...
            if self.center.device != _t.device:
                self.center = self.center.to(_t.device)
